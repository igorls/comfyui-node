...
interface MemoryQueueEntry {
  ...
  priority: number;
  availableAt: number;
  sequenceNumber: number; // 🎯 Garante FIFO perfeito mesmo com timestamps idênticos
  dequeuedAt?: number; // 🎯 Timestamp de quando foi retirado da fila (para tracking)
  checkpointKey: string; // 🎯 Chave do checkpoint para roteamento (ex: "epicrealism", "japanese", "default")
}
...
export class MemoryQueueAdapter implements QueueAdapter {
  ...
  private nextSequenceNumber = 0; // 🎯 Contador global para ordem de enfileiramento
  ...
  async reserve(opts?: { availableCheckpoints?: string[] }): Promise<QueueReservation | null> {
    ...
    const checkpointKeysToSearch = availableCheckpoints.length > 0
      ? availableCheckpoints.map(ckpt => 
        ...
        ).concat(['default']) // Sempre inclui jobs sem checkpoint específico
    ...
    if (lastSeq !== undefined && selectedEntry.sequenceNumber < lastSeq) {
      console.warn(
        `⚠️ FIFO VIOLATION DETECTED! Checkpoint: ${selectedCheckpointKey}, ` +
