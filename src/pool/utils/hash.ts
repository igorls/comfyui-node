...
function structuralJson(workflow: object): string {
  const nodes = workflow as Record<string, any>;
...
function canonicalInput(value: unknown): unknown {
  ...
  if (typeof value === "string") return RESOURCE_VALUE.test(value) ? value : "";
  // Nested object input (rare) → recurse structurally.
  if (value !== null && typeof value === "object") {
    const out: Record<string, unknown> = {};
    for (const k of Object.keys(value as Record<string, unknown>).sort()) {
      out[k] = canonicalInput((value as Record<string, unknown>)[k]);
    }
    return out;
  }
