...
export class JobsFeature extends FeatureBase {
     ...
     */
    async cancelJob(jobId: string): Promise<QueueManageResponse> {
     ...
     */
    async cancelJobs(jobIds: string[]): Promise<QueueManageResponse> {
     ...
     */
    async clearPendingJobs(): Promise<QueueManageResponse> {
        const response = await this.client.fetchApi("/api/queue", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
     ...
     */
    async interruptRunningJobs(): Promise<void> {
