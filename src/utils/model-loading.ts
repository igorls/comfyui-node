 ...
 * ```
 */
export function isModelLoadingNode(nodeType: string): boolean {
  return MODEL_LOADING_NODE_TYPES.includes(nodeType as any);
}
 ...
 * ```
 */
export function workflowContainsModelLoading(workflow: object): boolean {
 ...
 * ```
 */
export function getModelLoadingNodes(workflow: object): Array<{ nodeId: string; nodeType: string }> {
  const modelNodes: Array<{ nodeId: string; nodeType: string }> = [];
 ...
 * ```
 */
export function estimateModelLoadingTime(nodeType: string, storageType: StorageType = "ssd"): number {
 ...
 * ```
 */
export function estimateWorkflowModelLoadingTime(workflow: object, storageType: StorageType = "ssd"): number {
  const modelNodes = getModelLoadingNodes(workflow);
  let totalTime = 0;
 ...
 * ```
 */
export function getTimeoutMultiplierForModelLoading(
  workflow: object,
  hasExecutedBefore: boolean = false
): number {
