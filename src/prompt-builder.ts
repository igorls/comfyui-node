...
export class PromptBuilder<I extends string, O extends string, T extends NodeData> {
  ...
  validateOutputMappings(): this {
    ...
    const promptAny = this.prompt as any;
  ...
  validateNoImmediateCycles(): this {
    const promptAny2 = this.prompt as any;
    for (const [nodeId, nodeAny] of Object.entries(promptAny2)) {
      const inputs = (nodeAny as any)?.inputs || {};
  ...
  static fromJSON<I extends string, O extends string, T extends NodeData>(data: any): PromptBuilder<I,O,T> {
    const keysIn = Object.keys(data.mapInputKeys || {}) as I[];
    const keysOut = Object.keys(data.mapOutputKeys || {}) as O[];
    const builder = new PromptBuilder<I,O,T>(data.prompt, keysIn, keysOut);
