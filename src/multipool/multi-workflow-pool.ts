...
export class MultiWorkflowPool {
   ...
   */
  async submitToVariants(variants: Workflow<any>[], options?: SubmitJobOptions): Promise<string> {
    ...
    let chosen: Workflow<any> | undefined;
    let bestPriority = -Infinity;
    for (const variant of variants) {
      if (!variant.structureHash) variant.updateHash();
      ...
      if (client) {
        ...
        if (p > bestPriority) { bestPriority = p; chosen = variant; }
      }
    }
   ...
   */
  private triggerQueuesForIdleClient(client: EnhancedClient) {
    const queueKeys = new Set<string>(client.workflowAffinity ?? []);
