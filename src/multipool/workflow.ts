...
export class WorkflowJob<R extends WorkflowResult = WorkflowResult> {
  ...
  on<K extends EventKey<R>>(evt: K, fn: WorkflowJobEvents<R>[K]) {
    this.emitter.on(evt, fn as any);
    return this;
  }

  off<K extends EventKey<R>>(evt: K, fn: WorkflowJobEvents<R>[K]) {
    this.emitter.off(evt, fn as any);
    return this;
  }
  ...
  _emit<K extends EventKey<R>>(evt: K, ...args: Parameters<WorkflowJobEvents<R>[K]>) {
    this.emitter.emit(evt, ...args);
  }

  _finish(data: R) {
    this.doneResolve(data);
    this.emitter.emit("finished", data, (data as any)._promptId);
  }

  _fail(err: Error, promptId?: string) {
    this.doneReject(err);
...
export class Workflow<T extends WorkflowJSON = WorkflowJSON, O extends OutputMap = {}> {
  ...
  private _pendingImageInputs: Array<{
    nodeId: string;
    inputName: string;
    blob: Blob;
    fileName: string;
  ...
  async uploadAssets(api: ComfyApi): Promise<void> {
