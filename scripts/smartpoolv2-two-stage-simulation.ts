import { JobRecord, SmartPoolV2 } from "../src/index.ts";
...
if (generationNegatives.length === 0) {
    generationNegatives.push(
        "lowres, blurry, bad anatomy, extra limbs, watermark, text, signature, nsfw",
        "poor lighting, washed out colors, distorted perspective, nsfw",
        "cropped face, missing fingers, artifacts, posterization, nsfw",
        "muted colors, flat lighting, repetitive patterns, nsfw"
    );
}
...
/**
 * Helper to wait for SmartPoolV2 job completion
 */
function waitForSmartPoolV2Job(pool: SmartPoolV2, jobId: string): Promise<JobRecord> {
...
async function runSimulation() {
    log("blue", `[SmartPoolV2 Two-Stage Simulation] Starting with ${hosts.length} hosts: ${hosts.join(", ")}`);
    log("blue", `Generation host: ${GEN_HOST}, Edit hosts: ${EDIT_HOSTS.join(", ")}`);

    const pool = new SmartPoolV2(hosts);
    await pool.connect();

    // Set affinity: generation jobs go to GEN_HOST, edit jobs go to EDIT_HOSTS
    // Workflow hash automatically becomes the group ID
    ...
    log("green", `[SmartPoolV2 Two-Stage Simulation] Affinities configured`);
    log("green", `  Generation: ${GEN_HOST}`);
    log("green", `  Edit: ${EDIT_HOSTS.join(", ")}`);
    ...
    pool.on("job:failed", (e: any) => {
        ...
        log("red", `[✗ Failed ${failCount}] Job ${job.jobId.substring(0, 8)}... failed: ${errorMsg}`);
    });
    ...
    const producerTask = (async () => {
        try {
            while (enqueueGenContinuously && Date.now() < runtimeEnd) {
                ...
                // Fire-and-forget: wait for generation in background
                waitForSmartPoolV2Job(pool, genJobId)
                    .then((genJob) => {
            ...
            while (Date.now() < runtimeEnd) {
                ...
                try {
                    ...
                    try {
                        ...
                        const uploadName = `two-stage-v2-${Date.now()}-${Math.random().toString(16).slice(2)}.png`;
                        ...
                        log("cyan", `[Edit] Enqueued edit job ${editJobId.substring(0, 8)}... on ${targetEditClientId}`);
                        ...
                        waitForSmartPoolV2Job(pool, editJobId)
                            .then(() => {
                                log("green", `[✓ Edit Success] Job ${editJobId.substring(0, 8)}... completed`);
                            })
                            .catch((failedJob) => {
                                const errorMsg = failedJob?.lastError?.message || failedJob?.error?.message || String(failedJob);
                                log("red", `[Edit] Edit job ${editJobId.substring(0, 8)}... failed: ${errorMsg}`);
                            });
                    } catch (uploadError) {
    ...
    try {
        ...
        console.log("\n" + "=".repeat(80));
        log("bold", `[SmartPoolV2 Two-Stage Simulation] Final Summary`);
...
runSimulation().catch((error) => {
    log("red", `[SmartPoolV2 Two-Stage Simulation] Fatal error: ${error}`);
    process.exit(1);
});
