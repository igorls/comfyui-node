...
export class FileFeature extends FeatureBase {
     ...
     */
    async uploadImage(file, fileName, config) {
        const formData = new FormData();
        const fileBlob = file instanceof Buffer ? new Blob([new Uint8Array(file)]) : file;
     ...
     */
    async uploadMask(file, originalRef) {
        const formData = new FormData();
        const fileBlob = file instanceof Buffer ? new Blob([new Uint8Array(file)]) : file;
    ...
    async moveUserData(source, dest, options = { overwrite: false }) {
        return this.client.fetchApi(`/userdata/${encodeURIComponent(source)}/move/${encodeURIComponent(dest)}?overwrite=${options.overwrite}`, {
            method: "POST"
        });
    }
    /**
    ...
    async listUserData(dir, recurse, split) {
        const response = await this.client.fetchApi(`/userdata?${new URLSearchParams({
            dir,
