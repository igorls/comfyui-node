...
describe("JobStateRegistry", () => {
    ...
    describe("retention", () => {
        it("evicts the oldest terminal jobs beyond completedJobRetention (#13)", () => {
            jobRegistry.pool.options.completedJobRetention = 2;
            const ids = [0, 1, 2].map(() => jobRegistry.addJob(new Workflow({})));
            for (const id of ids)
                jobRegistry.setJobFailure(id, {});
        ...
        it("retains everything when completedJobRetention is 0", () => {
            jobRegistry.pool.options.completedJobRetention = 0;
            const ids = [0, 1, 2, 3].map(() => jobRegistry.addJob(new Workflow({})));
            for (const id of ids)
                jobRegistry.setJobFailure(id, {});
            for (const id of ids)
                expect(jobRegistry.jobs.has(id)).toBe(true);
        });
    });
});
