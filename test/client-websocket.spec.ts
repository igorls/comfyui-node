...
function jsonMsg(type: string, data: any) { return JSON.stringify({ type, data }); }

describe("ComfyApi WebSocket integration (simulated)", () => {
  test("message dispatch + close triggers reconnect attempt log", () => {
    const api: any = new ComfyApi("http://localhost:8188");

    const events: Record<string, number> = {};
    api.on("all" as any, () => { events.all = (events.all || 0) + 1; });
    api.on("reconnected" as any, () => { events.reconnected = (events.reconnected || 0) + 1; });
    api.on("status" as any, () => { events.status = (events.status || 0) + 1; });
    ...
    const logs: string[] = [];
    api.on("log" as any, (e: any) => { if (e.detail.message.includes("Attempt")) logs.push(e.detail.message); });
    sock.onclose?.({ type: "close" });
